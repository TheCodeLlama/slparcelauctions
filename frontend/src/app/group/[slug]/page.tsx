...
/**
 * Render at request time, never at build time. Mirrors the home page's
 * posture (see `frontend/src/app/page.tsx`). Static prerendering would
 * couple the Amplify build to whatever the backend happens to return at
 * build time — a bad-shape group payload would crash the build and
 * block every other page from deploying. Group membership, dissolution
 * state, and member rosters change per visit; a static snapshot is
 * never the right cache.
 */
...
async function fetchGroupBySlug(slug: string): Promise<FetchOutcome> {
  try {
    ...
    if (err.status === 410) {
      ...
      // We read defensively: the keys are not guaranteed across versions.
...
/**
 * Public realty-group profile. Server component — anonymous-safe by
 * construction (the underlying endpoint is `permitAll`), so SSR fetches
 * without a JWT just work. Logged-in member affordances are layered on
 * top via the {@link EditGroupAffordance} client component.
 *
 * Section composition: hero -> leader -> agents (when present).
 * Reserved sections for listings / sales / rating are intentionally
 * absent until sub-projects C/F land — there is no skeleton placeholder
 * for them on the public page.
 */
