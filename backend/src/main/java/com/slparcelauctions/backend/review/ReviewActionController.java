 ...
 * {@code /api/v1/reviews/{publicId}/*} rather than
 * {@code /api/v1/auctions/{publicId}/reviews}; keeping the two classes separate
 ...
 * {@code UserDetails}.
 *
 * <p>{@code publicId} in the path is the review's UUID — internal Long PKs
...
public class ReviewActionController {
    ...
    public ResponseEntity<ReviewResponseDto> respond(
        ...
        Long reviewId = resolveReviewId(publicId);
    ...
    private Long resolveReviewId(UUID publicId) {
        return reviewRepository.findByPublicId(publicId)
                .map(Review::getId)
                .orElseThrow(() -> new ReviewNotFoundException(publicId));
    }
}
