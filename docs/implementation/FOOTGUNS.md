...
### F.46 Bot endpoints ship without authentication in sub-spec 1 — Epic 06 MUST add it (resolved)

**Status (2026-05-16):** Resolved. Epic 06 Task 3 wired bearer-token auth via `BotSharedSecretAuthorizer` (`MessageDigest.isEqual` constant-time compare against `slpa.bot.shared-secret`), gated by `SecurityConfig.requestMatchers("/api/v1/bot/**").access(...)`. `BotStartupValidator` fails fast on blank / dev-placeholder / sub-16-char secrets in non-dev profiles. The ownership-only verification refactor (spec `2026-05-16`) further retired the VERIFY / MONITOR_AUCTION / MONITOR_ESCROW callbacks that this entry originally worried about; the surviving `/api/v1/bot/**` endpoints are still bearer-token gated.

**Why (historical):** `SecurityConfig` originally had `permitAll` on `GET /api/v1/bot/tasks/pending` and `PUT /api/v1/bot/tasks/{taskId}` because the SL bot worker did not exist yet. The body-level validation (`authBuyerId == slpa.bot-task.primary-escrow-uuid`, `salePrice == sentinelPrice`) was a temporary stopgap.
...
When a Java enum gains a new value (e.g., adding a new `BotTaskType` such
as `WITHDRAW_GROUP`), Hibernate's `update` mode does NOT rewrite the
existing Postgres CHECK constraint. Inserts with the new value fail at the
DB level with `check constraint violated` until the constraint is manually
refreshed.
