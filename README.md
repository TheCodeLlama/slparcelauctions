...
```bash
cd backend && ./mvnw test             # ~157 unit / slice / integration tests incl. JWT auth, verification, SL verification, dev simulate, the /api/v1 prefix migration smoke test, the S3 object storage unit tests, and the AvatarImageProcessor fixture-driven tests (integration tests need postgres on :5432 and MinIO on :9000)
